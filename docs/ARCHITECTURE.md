...
2. **Theme selection**: Clicking a theme row calls `setTheme(file, { patternFile })` → `loadTheme(file)` → cache check → `resolveTheme(file)` → fetch raw `.ovt` → `parseOVT()` → recursively resolve `extends:` chain → merge variables → `applyThemeState(file, theme, patternFile)` applies the theme, updates the palette/status, syncs the pattern select (via a dispatched `change` event that also drives the OBS Appearance mirror and ambient generator), and toggles the active theme row. The swap is committed atomically through the View Transition API when available, with a GSAP fade as the fallback. The slideshow advances through shuffle bags for both theme and pattern (current pick excluded, `pattern.svg` skipped) so neither repeats until the bag is exhausted. A 12s cadence (`SHUFFLE_MS`) drives it; the circular shuffle button skips to the next theme immediately, the `❚❚` button beside the theme name toggles pause/resume, and a thin linear progress bar (plus the ring) shows time until the next change.
