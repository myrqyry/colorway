...
1. **Page load**: `renderApp()` renders the shell, including a fixed full-viewport intro overlay (`#colorway-intro`) with a large per-character "Colorway" word (manual char spans, no SplitText). `runIntro()` starts `preloadAllThemes()` (chunks all 150 themes, fetches each `.ovt` from `public/themes/`, parses via `parseOVT()`, resolves inheritance chains, extracts a 6-color palette preview) and the GSAP `playColorwayIntroOpening()` concurrently. Only when both settle (`Promise.all`) does the app prepare the initial random theme under the overlay (`setTheme(initialTheme, { transition: false, animateTitle: false })`), init the workbench, then hand the settled word off into the sticky header title via `handoffColorway()` before `startShuffle()` begins.
2. **Theme selection**: Clicking a theme row calls `setTheme(file, { patternFile })` → `loadTheme(file)` → cache check → `resolveTheme(file)` → fetch raw `.ovt` → `parseOVT()` → recursively resolve `extends:` chain → merge variables → `applyThemeState(file, theme, patternFile)` applies the theme, updates the palette/status, syncs the pattern select (via a dispatched `change` event that also drives the OBS Appearance mirror and ambient generator), and toggles the active theme row. The swap is committed atomically through the View Transition API when available, with a GSAP fade (`crossfadeShowcase`, promise-based) as the fallback; `transition: false` skips animation entirely. After the transition, `animateHeaderColorway()` plays a small one-pass 3D roll on the header title. The slideshow advances through shuffle bags for both theme and pattern (current pick excluded, `pattern.svg` skipped) so neither repeats until the bag is exhausted. A 12s cadence (`SHUFFLE_MS`) drives it; the circular shuffle button skips to the next theme immediately, the `❚❚` button beside the theme name toggles pause/resume, and a thin linear progress bar (plus the ring) shows time until the next change.
